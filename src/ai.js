...
const { getMKHandle, getFactionHandle, getShortFactionName } = require('./data/mkHandles');
...
═══ כללים נוספים ═══
• ללא אימוג'ים בשום מקרה — לא 🧵 ולא שום סמל אחר
...
async function generateTweetPair(item, dayType, config, validIds) {
  ...
  // Build MK and party handle context from item data
  const mkName = item.mkName ?? item.initiator ?? null;
  const mkHandle = mkName ? getMKHandle(mkName) : null;
  const factionResult = item.factionName ? getFactionHandle(item.factionName) : null;
  const partyHandle = factionResult?.handle ?? null;
  const shortParty = item.factionName ? getShortFactionName(item.factionName) : null;

  const mkLine = mkName
    ? `ח"כ הרלוונטי: ${mkName}${shortParty ? ` (${shortParty})` : ''}` +
      (mkHandle ? ` — ידית טוויטר: ${mkHandle}` : '') +
      (partyHandle ? ` | מפלגה: ${partyHandle}` : '')
    : null;

  // Build mandatory MK mention line
  let mkMentionFormat = null;
  if (mkName) {
    mkMentionFormat = `ח"כ ${mkName}`;
    if (shortParty) mkMentionFormat += ` (${shortParty})`;
    if (mkHandle)   mkMentionFormat += ` ${mkHandle}`;
    if (partyHandle) mkMentionFormat += ` ${partyHandle}`;
  }
...
כללי חובה:
- ללא אימוג'ים בכלל
- חייב לכלול עובדה מספרית אחת מהנתונים
${mkMentionFormat ? `- כאשר מזכירים את הח"כ — השתמש בפורמט הזה בדיוק: ${mkMentionFormat}` : '- אסור לנחש ידיות טוויטר'}
- אסור לציין מידע שאינו בנתונים
- שורת האשטגים האחרונה חייבת להתחיל ב-@ערוץ_הכנסת ואחריה #כנסת + האשטג רלוונטי
- אם isNewBill=false — הצג כ"הצעה ישנה שעלתה לשלב חדש": ציין את statusDesc (מה השלב הנוכחי) ואת submissionDate (מתי הוגשה לראשונה). אל תכתוב "היום" — השתמש ב-activityDate כתאריך הפעילות האחרונה
- אם isNewBill=true — הצג כהצעה שהוגשה לאחרונה
- אל תכתוב "מתאריך [תאריך]" — כתוב מה קרה: "עלתה לקריאה שנייה", "הוגשה", "אושרה כחוק" לפי statusDesc
- דוגמת מבנה להצעה ישנה: "הצעת חוק [שם] של ח"כ X נמצאת ב[statusDesc] (פעילות אחרונה: [activityDate]). הוגשה ב-[submissionDate]."
...
אם אין מספיק מידע — החזר: null`;
  ...
  const linkUrl = item.docUrl ?? item.endpoint ?? null;
  const linkLine = linkUrl ? `\nבסוף הציוץ הוסף שורה עם הקישור: ${linkUrl}` : '';
...
עד 500 תווים. ללא האשטגים. ללא @mentions. ללא "לפרטים נוספים: knesset.gov.il".${linkLine}`;
